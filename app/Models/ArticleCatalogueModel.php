...
/**
 * ArticleCatalogueModel - Manages article categories and classifications
 * 
 * This model handles article catalogues including hierarchical structures,
 * SEO optimization, and relationship management with articles.
 * 
 * @package App\Models
 * @author System
 */
class ArticleCatalogueModel extends Model
{
    protected $table = 'article_catalogue';
    ...
    protected $validationRules = [
        ...
        'canonical' => 'required|min_length[3]|max_length[255]|is_unique[article_catalogue.canonical,id,{id}]',
    ...
    /**
     * Get catalogues with article count
     * 
     * @param int $limit The limit of results
     * @return array The catalogues with article count
     */
    public function getCataloguesWithArticleCount(int $limit = 0): array
    {
        $builder = $this->select('article_catalogue.*, COUNT(articles.id) as article_count')
                        ->join('articles', 'articles.article_catalogue_id = article_catalogue.id', 'left')
                        ->where('article_catalogue.publish', 1)
                        ->groupBy('article_catalogue.id')
                        ->orderBy('article_catalogue.sort_order', 'ASC')
                        ->orderBy('article_catalogue.title', 'ASC');
