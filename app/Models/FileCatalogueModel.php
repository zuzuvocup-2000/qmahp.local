...
/**
 * FileCatalogueModel - Manages file categories and organization
 * 
 * This model handles file categorization, organization, and hierarchical
 * structure for efficient file management and retrieval.
 * 
 * @package App\Models
 * @author System
 */
class FileCatalogueModel extends Model
{
    protected $table = 'file_catalogue';
    ...
    protected $validationRules = [
        ...
        'slug' => 'required|max_length[255]|is_unique[file_catalogue.slug,id,{id}]',
    ...
    /**
     * Get catalogues with file count
     * 
     * @param int $limit The limit of results
     * @return array The catalogues with file count
     */
    public function getCataloguesWithFileCount(int $limit = 0): array
    {
        $builder = $this->select('file_catalogue.*, COUNT(file.id) as file_count')
                        ->join('file', 'file.file_catalogue_id = file_catalogue.id', 'left')
                        ->where('file_catalogue.publish', 1)
                        ->where('file_catalogue.status', 1)
                        ->groupBy('file_catalogue.id')
                        ->orderBy('file_catalogue.sort_order', 'ASC')
                        ->orderBy('file_catalogue.title', 'ASC');
    ...
    /**
     * Get catalogues by parent with file count
     * 
     * @param int $parentId The parent ID
     * @param int $limit The limit of results
     * @return array The catalogues with file count
     */
    public function getCataloguesByParentWithFileCount(int $parentId, int $limit = 0): array
    {
        $builder = $this->select('file_catalogue.*, COUNT(file.id) as file_count')
                        ->join('file', 'file.file_catalogue_id = file_catalogue.id', 'left')
                        ->where('file_catalogue.parent_id', $parentId)
                        ->where('file_catalogue.publish', 1)
                        ->where('file_catalogue.status', 1)
                        ->groupBy('file_catalogue.id')
                        ->orderBy('file_catalogue.sort_order', 'ASC')
