...
/**
 * TourCatalogueModel - Manages tour categories and organization
 * 
 * This model handles tour categorization, organization, and hierarchical
 * structure for efficient tour management and retrieval.
 * 
 * @package App\Models
 * @author System
 */
class TourCatalogueModel extends Model
{
    protected $table = 'tour_catalogue';
    ...
    protected $validationRules = [
        ...
        'slug' => 'required|max_length[255]|is_unique[tour_catalogue.slug,id,{id}]',
        ...
        'tour_type' => 'permit_empty|max_length[100]',
    ...
    protected $validationMessages = [
        ...
        'tour_type' => [
            'max_length' => 'Tour type cannot exceed 100 characters'
        ],
        'status' => [
    ...
    /**
     * Get catalogues by tour type
     * 
     * @param string $tourType The tour type
     * @param int $limit The limit of results
     * @param int $offset The offset for pagination
     * @return array The catalogues
     */
    public function getCataloguesByTourType(string $tourType, int $limit = 0, int $offset = 0): array
    {
        $builder = $this->where('tour_type', $tourType)
                        ->where('publish', 1)
                        ->where('status', 1)
                        ->orderBy('sort_order', 'ASC')
                        ->orderBy('title', 'ASC');
    ...
    /**
     * Get catalogues with tour count
     * 
     * @param int $limit The limit of results
     * @return array The catalogues with tour count
     */
    public function getCataloguesWithTourCount(int $limit = 0): array
    {
        $builder = $this->select('tour_catalogue.*, COUNT(tour.id) as tour_count')
                        ->join('tour', 'tour.tour_catalogue_id = tour_catalogue.id', 'left')
                        ->where('tour_catalogue.publish', 1)
                        ->where('tour_catalogue.status', 1)
                        ->groupBy('tour_catalogue.id')
                        ->orderBy('tour_catalogue.sort_order', 'ASC')
                        ->orderBy('tour_catalogue.title', 'ASC');
    ...
    /**
     * Get catalogues by parent with tour count
     * 
     * @param int $parentId The parent ID
     * @param int $limit The limit of results
     * @return array The catalogues with tour count
     */
    public function getCataloguesByParentWithTourCount(int $parentId, int $limit = 0): array
    {
        $builder = $this->select('tour_catalogue.*, COUNT(tour.id) as tour_count')
                        ->join('tour', 'tour.tour_catalogue_id = tour_catalogue.id', 'left')
                        ->where('tour_catalogue.parent_id', $parentId)
                        ->where('tour_catalogue.publish', 1)
                        ->where('tour_catalogue.status', 1)
                        ->groupBy('tour_catalogue.id')
                        ->orderBy('tour_catalogue.sort_order', 'ASC')
     ...
     * 
     * @return array The available tour types
     */
    public function getAvailableTourTypes(): array
    {
        return $this->select('tour_type, COUNT(*) as count')
                    ->where('publish', 1)
                    ->where('status', 1)
    ...
    /**
     * Get catalogue count by tour type
     * 
     * @param string $tourType The tour type
     * @return int The catalogue count
     */
    public function getCatalogueCountByTourType(string $tourType): int
    {
        return $this->where('tour_type', $tourType)
                    ->where('publish', 1)
                    ->where('status', 1)
                    ->countAllResults();
    }
    ...
    /**
     * Get catalogues by tour type with count
     * 
     * @param string $tourType The tour type
     * @param int $limit The limit of results
     * @return array The catalogues with tour count
     */
    public function getCataloguesByTourTypeWithCount(string $tourType, int $limit = 0): array
    {
        $builder = $this->select('tour_catalogue.*, COUNT(tour.id) as tour_count')
                        ->join('tour', 'tour.tour_catalogue_id = tour_catalogue.id', 'left')
                        ->where('tour_catalogue.tour_type', $tourType)
                        ->where('tour_catalogue.publish', 1)
                        ->where('tour_catalogue.status', 1)
                        ->groupBy('tour_catalogue.id')
                        ->orderBy('tour_catalogue.sort_order', 'ASC')
    ...
    /**
     * Get catalogues with popular tours
     * 
     * @param int $limit The limit of results
     * @return array The catalogues with popular tours
     */
    public function getCataloguesWithPopularTours(int $limit = 0): array
    {
        $builder = $this->select('tour_catalogue.*, COUNT(tour.id) as tour_count, AVG(tour.rating) as avg_rating')
                        ->join('tour', 'tour.tour_catalogue_id = tour_catalogue.id', 'left')
                        ->where('tour_catalogue.publish', 1)
                        ->where('tour_catalogue.status', 1)
