...
/**
 * UserCatalogueModel - Manages user categories and role classifications
 * 
 * This model handles user catalogues including role definitions,
 * permissions, and hierarchical structures for user management.
 * 
 * @package App\Models
 * @author System
 */
class UserCatalogueModel extends Model
{
    ...
    protected $validationRules = [
        ...
        'canonical' => 'required|min_length[3]|max_length[255]|is_unique[user_catalogue.canonical,id,{id}]',
    ...
    protected $validationMessages = [
        ...
        'permissions' => [
            'permit_empty' => 'Permissions can be empty'
        ],
        'userid_created' => [
     ...
     * 
     * @param int $limit The limit of results
     * @return array The role catalogues
     */
    public function getRoleCatalogues(int $limit = 0): array
    {
        $builder = $this->where('publish', 1)
    ...
    /**
     * Get catalogues with user count
     * 
     * @param int $limit The limit of results
     * @return array The catalogues with user count
     */
    public function getCataloguesWithUserCount(int $limit = 0): array
    {
        $builder = $this->select('user_catalogue.*, COUNT(users.id) as user_count')
                        ->join('users', 'users.user_catalogue_id = user_catalogue.id', 'left')
                        ->where('user_catalogue.publish', 1)
                        ->groupBy('user_catalogue.id')
                        ->orderBy('user_catalogue.sort_order', 'ASC')
                        ->orderBy('user_catalogue.title', 'ASC');
    ...
    /**
     * Get catalogues by permissions
     * 
     * @param string $permission The permission to search for
     * @param int $limit The limit of results
     * @return array The catalogues with specified permission
     */
    public function getCataloguesByPermission(string $permission, int $limit = 0): array
    {
        $builder = $this->where('publish', 1)
                        ->like('permissions', $permission)
                        ->orderBy('sort_order', 'ASC')
                        ->orderBy('title', 'ASC');
