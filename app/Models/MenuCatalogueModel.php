...
/**
 * MenuCatalogueModel - Manages menu categories and classifications
 * 
 * This model handles menu catalogues including navigation structures,
 * hierarchical organization, and relationship management with menus.
 * 
 * @package App\Models
 * @author System
 */
class MenuCatalogueModel extends Model
{
    ...
    protected $validationRules = [
        ...
        'canonical' => 'required|min_length[3]|max_length[255]|is_unique[menu_catalogue.canonical,id,{id}]',
        ...
        'menu_type' => 'permit_empty|max_length[100]',
    ...
    protected $validationMessages = [
        ...
        'menu_type' => [
            'max_length' => 'Menu type cannot exceed 100 characters'
        ],
        'location' => [
    ...
    /**
     * Get catalogues by menu type
     * 
     * @param string $menuType The menu type to filter by
     * @param int $limit The limit of results
     * @return array The catalogues of specified menu type
     */
    public function getCataloguesByMenuType(string $menuType, int $limit = 0): array
    {
        $builder = $this->where('publish', 1)
    ...
    /**
     * Get catalogues by location
     * 
     * @param string $location The location to filter by
     * @param int $limit The limit of results
     * @return array The catalogues at specified location
     */
    public function getCataloguesByLocation(string $location, int $limit = 0): array
    {
        $builder = $this->where('publish', 1)
    ...
    {
        $builder = $this->select('id, title, parent_id, level, menu_type, location')
                        ->where('publish', 1)
                        ->orderBy('sort_order', 'ASC')
                        ->orderBy('title', 'ASC');
        ...
        foreach ($catalogues as $catalogue) {
            ...
            $suffix = '';
            if (!empty($catalogue['menu_type'])) {
                $suffix .= ' (' . $catalogue['menu_type'];
                if (!empty($catalogue['location'])) {
                    $suffix .= ' - ' . $catalogue['location'];
                }
                $suffix .= ')';
            }
    ...
    /**
     * Get catalogues with menu count
     * 
     * @param int $limit The limit of results
     * @return array The catalogues with menu count
     */
    public function getCataloguesWithMenuCount(int $limit = 0): array
    {
        $builder = $this->select('menu_catalogue.*, COUNT(menus.id) as menu_count')
                        ->join('menus', 'menus.menu_catalogue_id = menu_catalogue.id', 'left')
                        ->where('menu_catalogue.publish', 1)
                        ->groupBy('menu_catalogue.id')
                        ->orderBy('menu_catalogue.sort_order', 'ASC')
                        ->orderBy('menu_catalogue.title', 'ASC');
     ...
     * 
     * @return array The available menu types
     */
    public function getAvailableMenuTypes(): array
    {
        return $this->select('DISTINCT(menu_type) as menu_type')
                    ->where('publish', 1)
     ...
     * 
     * @return array The available locations
     */
    public function getAvailableLocations(): array
    {
        return $this->select('DISTINCT(location) as location')
                    ->where('publish', 1)
