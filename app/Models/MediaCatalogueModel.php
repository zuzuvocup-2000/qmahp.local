...
/**
 * MediaCatalogueModel - Manages media categories and organization
 * 
 * This model handles media categorization, organization, and hierarchical
 * structure for efficient media management and retrieval.
 * 
 * @package App\Models
 * @author System
 */
class MediaCatalogueModel extends Model
{
    protected $table = 'media_catalogue';
    ...
    protected $validationRules = [
        ...
        'slug' => 'required|max_length[255]|is_unique[media_catalogue.slug,id,{id}]',
        ...
        'media_type' => 'permit_empty|max_length[100]',
    ...
    protected $validationMessages = [
        ...
        'media_type' => [
            'max_length' => 'Media type cannot exceed 100 characters'
        ],
        'status' => [
     ...
     * @return array The catalogues
     */
    public function getCataloguesByMediaType(string $mediaType, int $limit = 0, int $offset = 0): array
    {
        $builder = $this->where('media_type', $mediaType)
                        ->where('publish', 1)
                        ->where('status', 1)
                        ->orderBy('sort_order', 'ASC')
                        ->orderBy('title', 'ASC');
    ...
    /**
     * Get catalogues with media count
     * 
     * @param int $limit The limit of results
     * @return array The catalogues with media count
     */
    public function getCataloguesWithMediaCount(int $limit = 0): array
    {
        $builder = $this->select('media_catalogue.*, COUNT(media.id) as media_count')
                        ->join('media', 'media.media_catalogue_id = media_catalogue.id', 'left')
                        ->where('media_catalogue.publish', 1)
                        ->where('media_catalogue.status', 1)
                        ->groupBy('media_catalogue.id')
                        ->orderBy('media_catalogue.sort_order', 'ASC')
                        ->orderBy('media_catalogue.title', 'ASC');
    ...
    /**
     * Get catalogues by parent with media count
     * 
     * @param int $parentId The parent ID
     * @param int $limit The limit of results
     * @return array The catalogues with media count
     */
    public function getCataloguesByParentWithMediaCount(int $parentId, int $limit = 0): array
    {
        $builder = $this->select('media_catalogue.*, COUNT(media.id) as media_count')
                        ->join('media', 'media.media_catalogue_id = media_catalogue.id', 'left')
                        ->where('media_catalogue.parent_id', $parentId)
                        ->where('media_catalogue.publish', 1)
                        ->where('media_catalogue.status', 1)
                        ->groupBy('media_catalogue.id')
                        ->orderBy('media_catalogue.sort_order', 'ASC')
     ...
     * 
     * @return array The available media types
     */
    public function getAvailableMediaTypes(): array
    {
        return $this->select('media_type, COUNT(*) as count')
                    ->where('publish', 1)
                    ->where('status', 1)
    ...
    /**
     * Get catalogue count by media type
     * 
     * @param string $mediaType The media type
     * @return int The catalogue count
     */
    public function getCatalogueCountByMediaType(string $mediaType): int
    {
    ...
    /**
     * Get catalogues by media type with count
     * 
     * @param string $mediaType The media type
     * @param int $limit The limit of results
     * @return array The catalogues with media count
     */
    public function getCataloguesByMediaTypeWithCount(string $mediaType, int $limit = 0): array
    {
        $builder = $this->select('media_catalogue.*, COUNT(media.id) as media_count')
                        ->join('media', 'media.media_catalogue_id = media_catalogue.id', 'left')
                        ->where('media_catalogue.media_type', $mediaType)
                        ->where('media_catalogue.publish', 1)
                        ->where('media_catalogue.status', 1)
                        ->groupBy('media_catalogue.id')
                        ->orderBy('media_catalogue.sort_order', 'ASC')
