$(document).ready(function() {
	...
	$(document).on('keydown','td.price >input',function(e){
		let data = $(this).val();
		if(data == 0){
			let unicode=e.keyCode || e.which;
			if(unicode != 190){
	...
	//khi thay đổi hoặc ấn phím xong : nếu  =='' sẽ trở về không, nếu == NaN cũng về 0
	$(document).on('change keyup blur','td.price >input',function(){
		let data = $(this).val();
		if(data == '' ){
			$(this).val('0');
			return false;
...
function openKCFinderSingleImage(button) {
	let closest = $(button).closest('.ibox');
	window.KCFinder = {
		callBack: function(files) {
			window.KCFinder = null;
			closest.find('.local_result img').attr("src", files);
			closest.find('input[name="image"]').val(files);
			closest.find('input[name="icon"]').val(files);
		}
	};
	window.open(BASE_URL + 'plugin/kcfinder-3.12/browse.php?type=images&dir=images/public', 'kcfinder_image',
        'status=0, toolbar=0, location=0, menubar=0, directories=0, ' +
...
// js fix openKCFinderMulti

function openKCFinderMulti(button) {
	let target = $(button).closest('.row').find('.ck-editor').attr('id');

	window.KCFinder = {
		callBackMultiple: function(files) {
			window.KCFinder = null;
			for (var i = 0; i < files.length; i++){
				CKEDITOR.instances[target].insertHtml('<p><img src="'+files[i]+'" alt="'+files[i]+'"></p>');
				// textarea.value += files[i] + "\n";
			}
		}
	};
