$(document).ready(function(){
	...
	$(document).on('click' ,'#update_price_product' ,function(){
		...
		$('#ajax-content tr td .checkbox-item:checked').each(function() {
			...
			let price = _this.parents('tr').find('input[name="price"]').val();
	...
	$(document).on('change','.ishome',function(){
		...
		let formURL = 'product/ajax/product/ishome';
			$.post(formURL, {
				objectid: objectid},
				function(data){
	...
	$(document).on('change','.highlight',function(){
		let _this = $(this);
		let objectid = _this.attr('data-id');
		let formURL = 'product/ajax/product/highlight';
			$.post(formURL, {
				objectid: objectid},
				function(data){
