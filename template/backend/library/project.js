$(document).ready(function(){
	if($('#project_catalogue').length){
		select2($('#project_catalogue'));
	}
	if(typeof catalogueid !='undefined'  ){
		pre_select2('project_catalogue',catalogueid);
	}
	...
	$(document).on('change','.publish',function(){
		...
		let formURL = 'project/ajax/project/status';
			$.post(formURL, {
				objectid: objectid},
				function(data){
...
function get_list_object(param){
	let ajaxUrl = 'project/ajax/project/listproject';
	$.get(ajaxUrl, {
